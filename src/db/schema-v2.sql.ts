...
-- CHANGED: seq is now per-conversation, not global
CREATE TABLE IF NOT EXISTS conversation_events (
  conversation  INTEGER NOT NULL,
  turn          INTEGER NOT NULL,
  event         INTEGER NOT NULL,
  seq           INTEGER NOT NULL,  -- per-conversation sequence, starts at 1 for each conversation
  ...
  PRIMARY KEY(conversation, seq),  -- composite primary key
  FOREIGN KEY(conversation) REFERENCES conversations(conversation)
);

-- Composite index for fast scoped addressing
...
-- CHANGED: seq is now per-conversation
CREATE TABLE IF NOT EXISTS idempotency_keys (
  conversation      INTEGER NOT NULL,
  agent_id          TEXT NOT NULL,
  client_request_id TEXT NOT NULL,
  seq               INTEGER NOT NULL,            -- per-conversation seq of the event written
  created_at        TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ','now')),
