...
export function SaveBar({ onSave, onDelete, disabled, scenarioId }: { onSave?: () => void; onDelete?: () => void; disabled?: boolean; scenarioId?: string }) {
  return (
    <div className="sticky bottom-0 z-10 bg-panel/95 backdrop-blur border-t border-border px-3 py-2 flex items-center justify-between">
      <div className="text-xs text-muted">{scenarioId ? `Scenario: ${scenarioId}` : 'New scenario'}</div>
