...
export const useAppStore = create<Store>((set, get) => ({
  ...
  appendComposeIntent(text, attachments) {
    ...
    const pf: ProposedFact = { type:'compose_intent', composeId, text, attachments };
    stampAndAppend([pf], set, get);
  ...
  async approveAndSend(composeId, finality) {
    ...
    // update local task id if created on send
    set({ taskId: newTaskId });
    set({ pending: { ...pending, [messageId]: { composeId } } });
    ingestSnapshotIntoJournal(snapshot, set, get);
    // Kick the tick loop once after send
    get().onTick();
  },

  addUserGuidance(text) {
    const gid = rid('g');
    const pf: ProposedFact = { type:'user_guidance', gid, text };
    stampAndAppend([pf], set, get);
  },
